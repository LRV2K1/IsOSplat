...
std::
    ...
    torch::Tensor v_depth = torch::zeros({num_points, 1}, xys.options()); //new, todo
    ...
    nd_rasterize_backward_kernel<<<tile_bounds, block, shared_mem>>>(
        ...
        v_depth.contiguous().data_ptr<float>()  //new, todo
    );
...
std::
    ...
    torch::Tensor v_colors =
        torch::zeros({num_points, channels}, xys.options());
    torch::Tensor v_opacity = torch::zeros({num_points, 1}, xys.options());
    torch::Tensor v_depth = torch::zeros({num_points}, xys.options()); //new, todo
...
opening(
    ...
    int num_points = mask.size(0) * mask.size(1);
    const dim3 mask_size = {mask.size(1), mask.size(0), 1};
    const int kernel_size = kernel.size(0);
    ...
    erosion<<<
        ...
        kernel_size,
        mask_size,
        kernel.contiguous().data_ptr<bool>(),
        mask.contiguous().data_ptr<bool>(),
    ...
    dilation<<<
        ...
        kernel.contiguous().data_ptr<bool>(),
        out_mask.contiguous().data_ptr<bool>(),
        out_mask2.contiguous().data_ptr<bool>()
    );
...
filter(
    ...
    torch::Tensor out_mask = torch::zeros({mask.size(0), mask.size(1)}, kernel.options());
