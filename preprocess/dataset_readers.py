...
from preprocess.colmap_loader import read_extrinsics_text, read_intrinsics_text, qvec2rotmat, rotmat2qvec, \
...
def getNerfppNorm(cam_info: list[CameraInfo]) -> dict[str, np.ndarray | float]:
...
def read_colmap_camera_info(
    cam_extrinsics: dict[int, cl.Image], 
    cam_intrinsics: dict[int, cl.Camera], 
    img_path: Path, 
    ...
    for idx, key in enumerate(sorted(cam_extrinsics, key=lambda x:cam_extrinsics[x].name)):
        ...
        depthloss = 1e8
        if pcd is not None and depth_path is not None and idx in train_idx:
            ...
            source_depth = depth_map = np.load(depth_path / f"{image_name}_pred.npy")
...
def optimize_depth(
    ...
    target_masked = target[mask] * depth_weight_masked
    target_masked = target_masked[:, None]
    source_masked = (source[mask])[:, None]
    source_masked = torch.cat((source_masked, torch.ones_like(source_masked, device=device)), 1)

    source_masked_t = torch.transpose(source_masked, 0, 1)
    source_masked_p = torch.linalg.inv(torch.matmul(source_masked_t, dense_depths))
    source_masked_p = torch.matmul(source_masked_p, source_masked_t)

    x = torch.matmul(source_masked_p, target_masked)
    s, t = x[0, 0].item(), x[1, 0].item()

    source_hat = s * source_masked + t
    loss = torch.mean((target_masked - source_hat)**2)

    return s * (source.cpu().numpy()) + t, loss.item()


def fetchPly(path: Path) -> BasicPointCloud:
...
def storePly(path: Path, xyz: np.ndarray, rgb: np.ndarray, xyzerr: Optional[np.ndarray] = None):
...
def refineColmapWithIndex(path: Path, train_index: list[int]) -> tuple[Path, dict[int|str, cl.Image], dict[int, cl.Camera]]:
    """ result
...
def transformed_gtcams(gtcams: dict[int, cl.Image], tarcam5: doct[int, cl.Image]) -> tuple[dict[int|str, cl.Image], Tensor, Tensor]:
    gtcam5 = {}
    gtcam5_R, gtcam5_T = [], []
    tarcam5_R, tarcam5_T = [], []
...
def BundleAdjustment_ColmapWithIndex(path: Path, train_index: int) -> tuple[Path, dict[int|str, cl.Image], dict[int, cl.Camera]]:
    """ result
    path: tmp_fewshot
    'cam_extrinsics' and 'point3D.ply' contains the points observed in (at least 2) train-views 
    """
...
def unprojFullDepth(caminfo: list[CameraInfo], ply_path: Path, single: bool = False) -> BasicPointCloud:
...
def pick_idx_from_360(path: Path, train_idx: list[int], kshot: int, center: ndarray, num_trials: int = 100_000) -> list[int]:
    """
    [Taekkii]
...
def depth_minmax(depth_name: str) -> tuple[float, float]:
