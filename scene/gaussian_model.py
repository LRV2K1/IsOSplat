...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, device: torch.device):
        self.device = device
    ...
    def restore(self, model_args, training_args: GroupParams):
        (self.active_sh_degree, 
        self._xyz, 
        self._features_dc, 
        self._features_rest,
    ...
    def partial_restore(self, model_args):
        (self.active_sh_degree, 
        self._xyz, 
        self._features_dc, 
        self._features_rest,
    ...
    def get_colors(self, camera_position: Tensor) -> Tensor:
        view_dirs = camera_position.repeat(self._xyz.shape[0], 1) - self.get_xyz
        return self.sigmoid_activation(spherical_harmonics(self.active_sh_degree, view_dirs, self.get_features))
    
    @property
    def get_opacity(self) -> Tensor:
        return self.sigmoid_activation(self._opacity)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        fused_color = inverse_sigmoid(torch.tensor(np.float32(pcd.colors / 255), device=self.device))
        features = torch.zeros((fused_color.shape[0], (self.max_sh_degree + 1) ** 2, 3)).float().cuda()
        features[:, 0, :] = fused_color
        features[:, 1:, :] = 0.0

        print("Number of points at initialisation : ", fused_point_cloud.shape[0])

        dist2 = torch.clamp_min(_C.distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().cuda()), 0.0000001)
    ...
    def _prune_optimizer(self, mask: Tensor):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            stored_state = self.optimizer.state.get(group['params'][0], None)
    ...
    def densification_postfix(self, 
                              new_xyz: Tensor, 
                              new_features_dc: Tensor, 
                              new_features_rest: Tensor, 
    ...
    def densify_and_split(self, grads: Tensor, grad_threshold: float, scene_extent: float, N: int = 2) -> int:
    ...
    def densify_and_prune(self, max_grad: float, min_opacity: float, extent: float, max_screen_size: float) -> tuple[int, int, int]:
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        clones = self.densify_and_clone(grads, max_grad, extent)
        splits = self.densify_and_split(grads, max_grad, extent)
        ...
        culls = self._xyz[prune_mask].shape[0]
        self.prune_points(prune_mask)

        torch.cuda.empty_cache()
        return culls, clones, splits
        
    def add_densification_stats(self, viewspace_point_tensor: Tensor, viewspace_depth_tensor: Tensor, radii: Tensor):
        view_space_gradients = viewspace_point_tensor
        view_depth_gradients = viewspace_depth_tensor[:, None]
        ...
        self.xyz_gradient_accum[mask] += torch.norm(view_gradients[mask,:3], dim=-1, keepdim=True)
        self.denom[mask] += 1
